...
% LINFRAME Splits a signal in linearly spaced（间隔） frames.
...
%   (symmetric 对称) padtype（填充类型）.
%
%   x:       input signal
%   fhop:    hop length (default 128)  滑动长度
%   flen:    length of frame (if empty defaults to 2*fhop)  帧长
%   padtype: how to pad start/end of signal. Values: 'trunc','pad','sym'
%            defaults to 'pad'
% 
%   fx:      framed output signal. (frame per column)每列为一帧
...
case 'sym'
    % Pad with zeros both front and end symmetricaly头尾对称补0 (MDCT 50%) 
    if (isequal(frem,0))
        fnum    = fnum+2;
        fpad(1) = fhop;
        fpad(2) = fhop;
