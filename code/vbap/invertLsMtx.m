...
% Dimensionality, 2d or 3d VBAP   根据ls_groups判断是二维VBAP 还是三维VBAP
...
ls_dirs_rad = ls_dirs*pi/180;%角度转弧度

% Number of loudspeakers and loudspeaker groups
N_group = size(ls_groups, 1);

% Convert to cartesian coordinates  扬声器方位转化为直角坐标
if dim == 2
...
for n = 1:N_group
    ...
    layoutInvMtx(n, :) = tempInv; % store the vectorized inverse as a row the output  得到的逆矩阵转化为向量存储
end
