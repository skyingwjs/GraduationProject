...
% panning (VBAP) [ref.1参考文献1], VBAP-base spreading of a panned source [ref.2&3参考文献2和3], 
% and Multiple-direction amplitude panning (MDAP) [ref.3 参考文献3]. A function 
% implementing the variant vector-base intensity panning [ref.4 参考文献4] is also 
% included. Recently, both VBAP and VBIP have been additionaly used for the 
% design of robust ambisonic decoding matrices, see [ref.5&6 参考文献5和6].
%
...
% spreading of a panned source（虚拟声源的传播或移动）, construction of panning gain
% tables（获取增益表）, and panning a moving source（移动声源） in a real-time block processing context.
%
% The library contains the following main functions:
%   
%   findLsPairs:    find sorted loudspeaker pairs from loudspeaker
%                   directions (for 2D layouts) 二维扬声器配置 
%   findLSTriplets: find valid loudspeaker triangles（有效的扬声器三角形） from loudspeaker
%                   directions (for 3D layouts) 
%   invertLsMtx:    precompute inversion of matrix（逆矩阵） of loudspeaker triplets 
%                   or pairs, for use in VBAP（计算扬声器三角形或者扬声器对的逆矩阵）
%   getSpreadSrcDirs:   get auxiliary source directions around panning 
%                       direction, for source spreading and MDAP
%   vbap:   Return VBAP panning gains（获得VBAP增益矩阵） for multiple panning directions, with
...
% h = gcf; 
% h.Position(3:4) = 2*h.Position(3:4);
